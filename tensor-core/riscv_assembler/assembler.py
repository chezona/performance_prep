...
class Op():
    ...
    pc: int
    args: Tuple[Operand]
    destination: Union[Operand, None]
...
class Jalr(Op):
    args: Tuple[GPRegister, Immediate, GPRegister]  # (rd, imm, rs1)
    ...
    def parseoperands(self):
        # Matches: jalr $0, 0($1)
        match_jalr = r"^\s*\w+\s+(?P<rd>\$\d+)\s*,\s*(?P<imm>-?\d+)\((?P<rs1>\$\d+)\)"
        match = re.match(match_jalr, self.line)
        self.throw_parse_error(match is not None)
...
class LdM(Op):
    ...
    def parseoperands(self):
        match_ldm = r"^\s*(?P<opcode>\w+)\s+\$m(?P<dest>\d+)\s*,\s*(?P<imm>-?\w+)\(\$(?P<reg>\d+)\)"
        match = re.match(match_ldm, self.line)
        self.throw_parse_error(match is not None)
        self.args = (GPRegister(f"${match.group('reg')}"), Immediate(match.group('imm')))
        self.destination = MatRegister(f"$m{match.group('dest')}")
    
    def encode(self):
        opcode = 0b00000111
        imm = self.args[1].value & 0x3FF  # 10 bits
        self.bytes = (
            (self.destination.num << 26) |
            (self.args[0].num << 21) |
            (imm << 7) |
            opcode
        )
class StM(Op):
    ...
    def parseoperands(self):
        match_stm = r"^\s*(?P<opcode>\w+)\s+\$m(?P<mat>\d+)\s*,\s*(?P<imm>-?\w+)\(\$(?P<reg>\d+)\)"
        match = re.match(match_stm, self.line)
        self.throw_parse_error(match is not None)
        self.args = (GPRegister(f"${match.group('reg')}"), Immediate(match.group('imm')))
        self.destination = MatRegister(f"$m{match.group('mat')}")
    
    def encode(self):
        opcode = 0b00100111
        imm = self.args[1].value & 0x3FF
        self.bytes = (
            (self.destination.num << 26) |
            (self.args[0].num << 21) |
            (imm << 7) |
...
class Gemm(Op):
    ...
    def parseoperands(self):
        match_gemm = r"^\s*(?P<opcode>\w+)\s+\$m(?P<rd>\d+)\s*,\s*\$m(?P<rs1>\d+)\s*,\s*\$m(?P<rs2>\d+)\s*,\s*\$m(?P<rs3>\d+)"
        match = re.match(match_gemm, self.line)
        self.throw_parse_error(match is not None)
        self.args = (
            MatRegister(f"$m{match.group('rs1')}"),
            MatRegister(f"$m{match.group('rs2')}"),
            MatRegister(f"$m{match.group('rs3')}")
        )
        self.destination = MatRegister(f"$m{match.group('rd')}")
...
class AluImm(Op):
    ...
    def encode(self) -> int:
        opcode = int(RISCV.Opcode.ALU_IMM.value)
...
class Branch(Op):
    ...
    def parseoperands(self):
        # Match: beq $rs1, $rs2, label
        match = re.match(r"^\s*\w+\s+(\$\d+),\s*(\$\d+),\s*(\w+)", self.line)
        self.throw_parse_error(match is not None)

        rs1_str, rs2_str, label = match.groups()
        self.args = (
            GPRegister(rs1_str),   # rs1
            GPRegister(rs2_str),   # rs2
            Immediate(label)       # symbolic offset, resolved in fill_symbols
        )

    def encode(self):
        rs1 = self.args[0].num
        rs2 = self.args[1].num
        imm = self.args[2].value - self.pc  # Byte offset from current PC to label

        if imm % 2 != 0:
            raise ValueError(f"Unaligned branch offset: {imm}")

        # Shift right by 1 because LSB is always 0 in branch encoding (instruction alignment)
        # imm = imm_offset >> 1

        # Extract B-type immediate fields
        imm_12   = (imm >> 11) & 0x1
        imm_10_5 = (imm >> 5) & 0x3F
        imm_4_1  = (imm >> 1) & 0xF
        imm_11   = (imm >> 10) & 0x1

        # Get funct3 using the Funct3 enum
        # funct3_map = get
        funct3 = int(RISCV.get_funct3(self.opcode).value)

        opcode = int(RISCV.Opcode.BRANCH.value)
...
class Jr(Macro):
    def __init__(self, l, opcode, filename, line_number):
        ...
        self.parseoperands()
    def parseoperands(self):
        match_jr = r"^\s*\w+\s+(?P<reg>\$\d+)"
        match = re.match(match_jr, self.line)
        self.throw_parse_error(match is not None)
        self.args = (GPRegister(match.group("reg")),)
    def expand(self):
        reg = self.args[0]
        # Properly format the jalr equivalent line
        line = f"jalr $0, 0({reg})"
        jalr = Jalr(line, "jalr", self.filename, self.line_number)
        return [jalr]
...
class Renderer():
    ...
    def _write_binary(self):
        ...
        with open(self.output_filename, "r+b") as f, open("hex.txt", "w") as write:  # open once outside loop
            m = mmap.mmap(f.fileno(), 0)
            for op in self.ops: 
                print(f"{op.pc}:{op.pc+4} - {hex(op.bytes)}")
                instr = hex(op.bytes)[2:].zfill(8)
                print(instr)
                write.write(instr + "\n")  # write each instruction on a new line
                m[op.pc:op.pc+4] = op.bytes.to_bytes(4, byteorder='little')
            m.close()
    ...
    def _parseop(self, l: str, line_number: int) -> Union[Op,Symbol]:
